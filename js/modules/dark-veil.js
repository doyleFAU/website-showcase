/**
 * Subtle animated dark veil — adapted from @react-bits/DarkVeil-JS-CSS
 * https://reactbits.dev
 */

import { Renderer, Program, Mesh, Triangle, Vec2 } from "https://esm.sh/ogl@1.0.11";
...
export function createDarkVeil(container, options = {}) {
  ...
  mount.className = "darkveil-container";
  ...
  const resize = () => {
    ...
    renderer.setSize(width * settings.resolutionScale, height * settings.resolutionScale);
  ...
  const draw = () => {
    ...
    program.uniforms.uTime.value = ((performance.now() - startTime) / 1000) * settings.speed;
    applySettings();
    renderer.render({ scene: mesh });
  };
