/**
 * Interactive wave lines — adapted from @react-bits/Waves-JS-CSS
 * https://reactbits.dev
 */
...
export function createWaves(container, options = {}) {
  ...
  mount.className = "waves-container";
  ...
  const setLines = () => {
    ...
    const totalLines = Math.ceil(oWidth / settings.xGap);
    const totalPoints = Math.ceil(oHeight / settings.yGap);
    const xStart = (width - settings.xGap * totalLines) / 2;
    const yStart = (height - settings.yGap * totalPoints) / 2;

    for (let i = 0; i <= totalLines; i++) {
      const pts = [];
      for (let j = 0; j <= totalPoints; j++) {
        pts.push({
          x: xStart + settings.xGap * i,
          y: yStart + settings.yGap * j,
          wave: { x: 0, y: 0 },
