/**
 * Grainy gradient swirls — adapted from @react-bits/Grainient-JS-CSS
 * https://reactbits.dev
 */
...
export function createGrainient(container, options = {}) {
  ...
  mount.className = "grainient-container";
  ...
  const program = new Program(gl, {
    ...
    uniforms: {
      ...
      uGrainAnimated: { value: settings.grainAnimated ? 1.0 : 0.0 },
  ...
  const applySettings = () => {
    ...
    u.uColorBalance.value = settings.colorBalance;
    u.uWarpStrength.value = settings.warpStrength;
    u.uWarpFrequency.value = settings.warpFrequency;
    u.uWarpSpeed.value = settings.warpSpeed;
    u.uWarpAmplitude.value = settings.warpAmplitude;
    u.uBlendAngle.value = settings.blendAngle;
    u.uBlendSoftness.value = settings.blendSoftness;
    u.uRotationAmount.value = settings.rotationAmount;
    u.uNoiseScale.value = settings.noiseScale;
    u.uGrainAmount.value = settings.grainAmount;
    u.uGrainScale.value = settings.grainScale;
    u.uGrainAnimated.value = settings.grainAnimated ? 1.0 : 0.0;
    u.uContrast.value = settings.contrast;
    u.uGamma.value = settings.gamma;
    u.uSaturation.value = settings.saturation;
    u.uCenterOffset.value = new Float32Array([settings.centerX, settings.centerY]);
    u.uZoom.value = settings.zoom;
    u.uColor1.value = new Float32Array(hexToRgb(settings.color1));
    u.uColor2.value = new Float32Array(hexToRgb(settings.color2));
    u.uColor3.value = new Float32Array(hexToRgb(settings.color3));
  };
  ...
  const draw = (timestamp) => {
    ...
    program.uniforms.iTime.value = (timestamp - t0) * 0.001;
    renderer.render({ scene: mesh });
  };
  ...
  return {
    destroy() {
      ...
      try { mount.removeChild(gl.canvas); } catch { /* ignore */ }
