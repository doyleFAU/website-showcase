/**
 * Glitchy terminal digits — adapted from @react-bits/FaultyTerminal-JS-CSS
 * https://reactbits.dev
 */
...
export function createFaultyTerminal(host, options = {}) {
  if (!host) return null;
  ...
  const ditherValue =
    typeof settings.dither === "boolean" ? (settings.dither ? 1 : 0) : settings.dither;
  const tintVec = hexToRgb(settings.tint);

  const mount = document.createElement("div");
  mount.className = "faulty-terminal-container";
  host.appendChild(mount);

  const renderer = new Renderer({ dpr: settings.dpr, alpha: true });
  ...
  const draw = (timestamp) => {
    ...
    if (settings.pageLoadAnimation && loadAnimationStart === 0) {
      loadAnimationStart = timestamp;
    }

    if (!settings.pause) {
      const elapsed = (timestamp * 0.001 + timeOffset) * settings.timeScale;
    ...
    if (settings.pageLoadAnimation && loadAnimationStart > 0) {
      ...
      const animationElapsed = timestamp - loadAnimationStart;
